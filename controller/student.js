...
exports.getSpecificStudent = (req,res,next) =>{
    ...
    Student.find({reg_no:reg_no}).then((result)=>{
        if(!result){
...
exports.updateStudentPaymentStatus=(req,res,next)=>{
    ...
    .then(student=>{
        ...
        res.status(200).send({ message: "Student paid successfully!", student: result });
    })
    .catch(error => {
        if (!error.statusCode) {
...
exports.createStudent =(req,res,next)=>{
    ...
    const {faculty, school, programme, specialization, reg_no, student_name, gender, batch, credits, cgpa, remark } = req.body;
    const student = new Student({
        faculty, school, programme, specialization, reg_no, student_name, gender, batch, credits, cgpa, remark 
    });
...
exports.updateStudent=(req,res,next)=>{
    ...
    const {faculty, school, programme, specialization, student_name, gender, batch, credits, cgpa, remark } = req.body;
    Student.find({reg_no: reg_no})
        .then((student)=>{
            if(student.length==0){
                const error = new Error('Could not find student.');
            ...
            res.status(200).send({ message: "Student details updated successfully!", student: result });
        })
        .catch(error => {
            if (!error.statusCode) {
                error.statusCode = 500;
