...
exports.getDueByRegNo=(req,res,next)=>{
    ...
    Due.find({reg_no:reg_no}).then(result=>{
        if(!result){
            const error = new Error("Could not find student with registration number");
            error.status = 404;
            throw error;
        }
...
exports.createStudentDue=(req,res,next)=>{
    ...
    const due = new Due({
        reg_no,department,amount_due,details
    });
    due.save().then(result => {
        res.status(200).json({message:"Student Due created successfully", data: result});
    }).catch((error) => {
        if (!error.statusCode) {
...
exports.deleteStudentByRegAndDept=(req,res,next)=>{
    const {reg_no,department}= req.body;
    Due.find({reg_no,department}).then((result)=>{
        if (!result) {
